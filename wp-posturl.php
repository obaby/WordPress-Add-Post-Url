<?php
/*
Plugin Name: Add Post URL [增强版]
Plugin URI: http://easwy.com/blog/wordpress/wp-posturl/
Description: This plugin allows you to insert a user specific text (such as copyright, credit, advertisement, etc.) at the beginning/ending of your posts. You can also decide which posts display the specific text. 增加数字id链接功能 http://oba.by
Version: 2.1.0
Author: Easwy Yang,obaby
...
function wp_posturl($text) {
    ...
    if ((is_single() && $posturl_options['add_to_single'])
        ...
        if (!empty($header_text))
        {
            ...
            $header_text = str_replace("%post_id%", $post->ID, $header_text);
        ...
        if (!empty($footer_text))
        {
            ...
            $footer_text = str_replace("%post_id%", $post->ID, $footer_text);
